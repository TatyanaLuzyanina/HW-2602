class Student:
    ...
    def av_rate(self, course):
        if course in self.grades.keys():
            Av_rate_stud = sum(self.grades[course]) / len(self.grades[course])
...
#Задача 4:
list_students = [best_student, best_student1]
list_lectures = [lecture1, lecture2]

print("")
print("Задача 4:")
def av_rate_students(list, course):
    print("Студенты:")
    for i in list:
        if course in i.grades.keys():
            print(i.name + ": " + str(round(sum(i.grades[course]) / len(i.grades[course]), 1)))
av_rate_students(list_students, "Python")

def av_rate_lectures(list):
    print("Лекторы:")
    for i in list:
        print(i.name + ": " + str(round(sum(i.grades[i.courses_attached[0]]) / len(i.grades[i.courses_attached[0]]), 1)))
av_rate_lectures(list_lectures)
